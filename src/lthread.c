    ...
    do {                            \
        ...
        default:                    \
            ...
            break;                  \
                                    ...
                                    \
        ltprintsz = sprintf(tmp, ltprint, lt->id,   \
            state, lt->fd_wait, lt->state & bit(LT_SLEEPING),       \
            tick_diff_msecs(lt->birth, rdtsc()),    \
            lt->funcname);                          \
        if (sz - ltprintsz < 0) {                   \
            new_buf = realloc(buf, 1024);           \
            if (new_buf)                            \
                buf = new_buf;                      \
            else {                                  \
                free(buf);                          \
            ...
            sz += 1024;                             \
                                                    ...
                                                    \
        strcat(buf + total_strlen, tmp);            \
        total_strlen += ltprintsz;                  \
        sz -= ltprintsz;                            \
} while (0)                                         \

char *
print_lthreads_buf(struct rb_node *node, char **buf1, long *sz1, long *total_strlen1)
{
    char *buf = *buf1, *new_buf = NULL, *state = NULL;
    lthread_t *lt = NULL, *lttmp = NULL;
    long sz = *sz1, ltprintsz = 0, total_strlen = *total_strlen1;
    char tmp[128];
    char *ltprint = "<lt:%-10ld state:%-15.15s fd_wait:%-10d "
        " sleeping:%d age:%-10"PRIu64" function:%s>\n";
    sched_node_t *data = NULL;
    data = container_of(node, sched_node_t, node);
    LIST_FOREACH_SAFE(lt, &data->lthreads, sleep_next, lttmp) {
        UPDATE_BUF(lt);
    }
    *total_strlen1 = total_strlen;
    *sz1 = sz;
    if (node->rb_left)
        print_lthreads_buf(node->rb_left, buf1, sz1, total_strlen1);
    if (node->rb_right)
        print_lthreads_buf(node->rb_right, buf1, sz1, total_strlen1);
...
lthread_summary()
{
    char *buf = NULL, *new_buf = NULL, *state = NULL;
    lthread_t *lt = NULL, *lttmp = NULL;
    char tmp[128];
    char *ltprint = "<lt:%-10ld state:%-15.15s fd_wait:%-10d "
        " sleeping:%d age:%-10"PRIu64" function:%s>\n";
    long sz = 1024, ltprintsz = 0, total_strlen = 0;
    //int i = 0;

    if ((buf = calloc(1, 1024)) == NULL) {
        return NULL;
    }
    sprintf(buf, "lthread summary\n"
        "total lthreads sleeping: %-10d waiting:%-10d"
        " created: %-10d age: %"PRIu64" msecs\n\n",
        sched->sleeping_state,
        sched->waiting_state,
        sched->total_lthreads,
        tick_diff_msecs(sched->birth, rdtsc()));
    sz -= strlen(buf);


    /* print lthreads in new queue */
    LIST_FOREACH_SAFE(lt, &sched->new, new_next, lttmp) {
        UPDATE_BUF(lt);
    }

    /* print sleeping threads */
    if (sched->sleeping.rb_node) {
        print_lthreads_buf(sched->sleeping.rb_node, &buf, &sz, &total_strlen);
    }

    /* go through the fdmap table, and print lthreads that are not sleeping
     * but waiting on an fd event.
     */
    /*
    for (i = 0; i < MAX_FD; i++) {
        lt = sched->fdmap[i][0];
